        ...
        if (update.from_user.id == update.message.reply_to_message.from_user.id) or g:
            ...
            if len(cb_data) > 1:
                ...
                try:
                    ...
                    await i_m_s_e_g.edit_text("🔴 ғᴀɪʟᴇᴅ 🔴\n\n" + str(e) + "\n\n#error")
        else:
            await bot.answer_callback_query(
                callback_query_id=update.id,
                text="ᴡʜᴏ ᴀʀᴇ ʏᴏᴜ?",
            ...
            if len(g_del_list) != 0:
                for f in g_del_list:
                    if os.path.isfile(f):
                        ...
                        shutil.rmtree(f)
                await update.message.edit_text(f"⚫ ᴅᴇʟᴇᴛᴇᴅ ⚫\n\n🚮: {len(g_del_list)} ғɪʟᴇs\n\n#deleted")
            else:
                await update.message.edit_text("ɴᴏᴛʜɪɴɢ ᴛᴏ ᴄʟᴇᴀʀ")
        else:
            await update.message.edit_text("ʏᴏᴜ ɴᴏᴛ ᴀʟʟᴏᴡᴇᴅ")
    elif cb_data == "fuckoff":
        await bot.answer_callback_query(
            update.id, text="ᴛʀʏɪɴɢ ᴛᴏ ᴄᴀɴᴄᴇʟ", show_alert=False
        )
        await update.message.edit_text("ғɪɴᴇ")
